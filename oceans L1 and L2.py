...
def ocean_counter(heatmapav, heatmap, heatmapint, ocean_index, land_index):
    '''heatmapav.append(
        (np.sum(heatmap[ocean_index]) / np.sum(heatmapint[ocean_index])) / (
                    np.sum(heatmap[land_index]) / np.sum(heatmapint[land_index])))'''
    heatmapav.append(
...
heatmap1av, heatmap2av, heatmap1l2av, heatmap2l2av, heatmapmerg1av, heatmapmerg2av = [
], [], [], [], [], []
for i in range(40):
    ...
    heatmap_merg1 = heatmap1_orig[i:(
        180 - i), :] + heatmap1_L2_orig[i:(180 - i), :]
    heatmap_merg2 = heatmap2_orig[i:(
        180 - i), :] + heatmap2_L2_orig[i:(180 - i), :]
    ...
    heatmapmerg1av = ocean_counter(
        heatmapmerg1av, heatmap_merg1, heatmapint_new, ocean_index, land_index)
    heatmapmerg2av = ocean_counter(
        heatmapmerg2av, heatmap_merg2, heatmapint_new, ocean_index, land_index)
...
plt.plot(list(range(90, 50, -1)), heatmapmerg1av, 'b', label="Merged nominal")
plt.plot(list(range(90, 50, -1)), heatmapmerg2av,
...
plt.plot(list(range(90, 50, -1)), heatmapmerg1av,
         'b--', label="Merged nominal SAA excl.")
plt.plot(list(range(90, 50, -1)), heatmapmerg2av,
         'r--', label="Merged redundant SAA excl.")
